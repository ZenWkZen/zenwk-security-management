...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		// S el token es invaido con continua con el proceso de autorizacion de la sesion
		if (token == null) {
		...
		if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
			...
			if (userDetails == null || !jwtProvider.validateToken(token, username)) {
				jwtProvider.sendErrorResponse(response, HttpServletResponse.SC_UNAUTHORIZED,
						AuthEnums.AUTH_TOKEN_INVALID.getMessage());
				return;
			} else if (!validateRolUser(token, request)) {
				jwtProvider.sendErrorResponse(response, HttpServletResponse.SC_FORBIDDEN,
						AuthEnums.AUTH_URI_FORBIDEN.getMessage());
				return;
			} else {
	 ...
	 * 
	 * <p>
	 * <b> CU001_Seguridad_Creacion_Usuario </b> Establece la autenticación del
	 ...
	 * 
	 * <p>
	 * <b> CU001_Seguridad_Creacion_Usuario </b> Verifica si el usuario con rol USER
	 * tiene acceso a la URL solicitada.
	 * </p>
	 * </p>
	 * 
	 * @author <a href="alineumsoft@gmail.com">C. Alegria</a>
	...
	private boolean validateRolUser(String token, HttpServletRequest request) {
		...
		if (roles.contains(RoleEnum.USER.name()) && List.of(HttpMethodResourceEnum.USER_CREATE.getResource(),
				HttpMethodResourceEnum.PERSON_CREATE.getResource()).stream().anyMatch(uri::contains)) {
	...
	/**
	 * <p>
	 * <b> CU001_Seguridad_Creacion_Usuario </b> Actualizacion de
	 * UsernamePasswordAuthenticationToken con los permisos y metadatos
	 * </p>
	 * 
	 * @author <a href="alineumsoft@gmail.com">C. Alegria</a>
	...
	private void setAuthenticationDetails(UsernamePasswordAuthenticationToken authentication,
		...
		Map<String, Object> details = Map.of(WEB_DETAILS, new WebAuthenticationDetailsSource().buildDetails(request));
