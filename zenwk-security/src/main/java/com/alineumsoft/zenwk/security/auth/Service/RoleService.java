...
public class RoleService extends ApiRestSecurityHelper {
	...
	public Long createRole(RoleDTO dto, HttpServletRequest request, UserDetails userDetails) {
		...
		try {
			if (roleRepository.existsByName(dto.getName())) {
				throw new EntityExistsException(SecurityExceptionEnum.FUNC_ROLE_EXISTS.getCodeMessage(dto.getName()));
			}
	 ...
	 * @param userDetails
	 */
	public void updateRole(Long roleId, RoleDTO dto, HttpServletRequest request, UserDetails userDetails) {
		String username = userDetails.getUsername();
		LogSecurity logSec = initializeLog(request, username, getJson(dto), notBody,
				SecurityActionEnum.ROLE_UPDATE.name());
		try {
			Role roleTarget = findRoleById(roleId);
			if (ObjectUpdaterUtil.updateDataEqualObject(new Role(dto.getName(), dto.getDescription()), roleTarget)) {
	...
	public RoleDTO findRoleById(Long roleId, HttpServletRequest request, UserDetails userDetails) {
		String username = userDetails.getUsername();
		LogSecurity logSec = initializeLog(request, username, notBody, notBody, SecurityActionEnum.ROLE_GET.name());
		try {
	...
	/**
	 * <p>
	 * <b> CU001_Seguridad_Asignación_Roles </b> Busca todos los roles guardados en
	 * el sistema.
	 * </p>
	 * 
	 * @author <a href="alineumsoft@gmail.com">C. Alegria</a>
	 ...
	 * @return
	 */
	public PageRoleDTO findAllRoles(Pageable pageable, HttpServletRequest request, UserDetails userDetails) {
		String username = userDetails.getUsername();
		LogSecurity logSec = initializeLog(request, username, notBody, notBody, SecurityActionEnum.ROLE_LIST.name());
		try {
			int pageNumber = getPageNumber(pageable);
			// Consulta paginada y ordenamiento
			Page<Role> pageRoles = roleRepository.findAll(PageRequest.of(pageNumber, pageable.getPageSize(),
					pageable.getSortOr(Sort.by(Sort.Direction.ASC, ROlE_NAME))));
			// Se persiste log
			saveSuccessLog(HttpStatus.OK.value(), logSec, logSecRepo);
	...
	public PageRoleDTO getPageRolesDTO(Page<Role> pageRoles) {
		List<RoleDTO> roles = pageRoles.stream().map(role -> new RoleDTO(role)).collect(Collectors.toList());
		PaginatorDTO paginator = new PaginatorDTO(pageRoles.getTotalElements(), pageRoles.getTotalPages(),
	...
	/**
	 * <p>
	 * <b> CU001_Seguridad_Asignación_Roles </b> ELmina un rol almacenado en el
	 * sistema.
	 * </p>
	 * 
	 * @author <a href="alineumsoft@gmail.com">C. Alegria</a>
	...
	public void deleteRole(Long roleId, HttpServletRequest request, UserDetails userDetails) {
		...
		try {
			...
			if (e instanceof DataIntegrityViolationException) {
				throw new TechnicalException(SecurityExceptionEnum.TECH_ROLE_NOT_DELETE.getCodeMessage(), e.getCause(),
						logSecRepo, logSec);
			} else {
				throw new FunctionalException(e.getMessage(), e.getCause(), logSecRepo, logSec);
			}
	...
	public Role findRoleByName(String name) {
		return roleRepository.findByName(name).orElseThrow(
				() -> new EntityNotFoundException(CoreExceptionEnum.FUNC_COMMON_ROLE_NOT_EXIST.getCodeMessage(name)));
	}
	...
	/**
	 * <p>
	 * <b> CU002_Seguridad_Asignación_Roles </b> Obtien los roles asociados a un
	 * permiso persistido en el sistema.
	 * </p>
	 * 
	 * @author <a href="alineumsoft@gmail.com">C. Alegria</a>
	 * @param pageable
