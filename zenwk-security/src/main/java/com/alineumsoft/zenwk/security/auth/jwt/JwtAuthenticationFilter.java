...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			// S el token es invaido con continua con el proceso de autorizacion de la
			// sesion
			if (token == null) {
			...
			if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
				...
				if (userDetails == null || !jwtProvider.validateToken(token, username)) {
					jwtProvider.sendErrorResponse(username, request, response, HttpServletResponse.SC_UNAUTHORIZED,
							SecurityExceptionEnum.FUNC_AUTH_TOKEN_INVALID);
					return;
			...
			jwtProvider.sendErrorResponse(username, request, response, HttpServletResponse.SC_UNAUTHORIZED, e);
		}
	}
